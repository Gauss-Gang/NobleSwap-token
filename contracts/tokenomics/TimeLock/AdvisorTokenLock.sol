/*  _____________________________________________________________________________
    ...
    Gauss(Gang) Advisor Token Lock Contract
...
contract AdvisorTokenLock is Context, Ownable {
    ...
    constructor(IBEP20 token_, address sender_, address beneficiary_, uint256 amount_) onlyOwner() {
        ...
        _amount = amount_;
        _releaseTime = _advisorLockTimes[0];
    ...
    function release() public virtual {
        ...
        uint256 amount = _advisorTokenAmountsList[_lockCounter];
        require(amount > 0, "CommunityTokenLock: no tokens to release");

        token().transfer(beneficiary(), amount);
        
        _lockCounter = _lockCounter + 1;
        _releaseTime = _advisorLockTimes[_lockCounter];
    }
}