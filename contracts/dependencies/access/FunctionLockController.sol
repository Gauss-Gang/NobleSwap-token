...
*/
contract FunctionLockController {
    ...
    /*  Modifier to make a function callable only by a certain role.
            - In addition to checking the sender's role, `address(0)` 's role is also considered. 
    ...
    */
    ...
    // Contract might receive/hold BNB as part of the maintenance process.
    ...
    /*  Schedule an operation containing a single transaction. Emits a {CallScheduled} event.
    ...
    */
    ...
    /*  Schedule an operation containing a batch of transactions. Emits one {CallScheduled} event per transaction in the batch.
    ...
    */
    ...
    /*  Execute an (ready) operation containing a batch of transactions. Emits one {CallExecuted} event per transaction in the batch.
    ...
    */
    ...
    /* Changes the minimum timelock duration for future operations. Emits a {MinDelayChange} event.
            Requirements:   the caller must be the timelock itself. This can only be achieved by scheduling and later executing
